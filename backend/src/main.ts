...
router.get('/leases', async (ctx: Context) => {
  ...
  ctx.body = leases.map((lease) => ({
    ...
    auditTemplatePath: `http://localhost:3000/input/audit/${lease.templateAuditPath}`,
    excelTemplatePath: `http://localhost:3000/input/excel/${lease.templateExcelPath}`,
    excelPath: `http://localhost:3000/output/${lease.processId}.xlsx`,
    leasePath: `http://localhost:3000/output/${lease.id}.pdf`,
  }));
});
