...
export class AIProcessService {
    ...
    private async leaseGetExcel(leaseID: number, index: number, lease: string, excelScan: ExcelCase[]) {
        for (let use_case of excelScan) {
            ...
            const sheetValues = Object.keys(outerJson).map(key => ({
                ...
                page: parseInt(key[0]),
                value: !outerJson[key] || outerJson[key].length === 0 || outerJson[key] === "null" ? "Non indiqué" : outerJson[key]
            }))
    ...
    private async leaseGetAudit(leaseID: number, index: number, lease: string, auditScan: AuditCase[]) {
        ...
        for (let use_case of auditScan) {
            ...
            const entries = Object.keys(outerJson).map((key) => {
                ...
                return {
                    page: element?.page ?? 0,
                    text: !values || values.length === 0 || values === "null"  ? "Aucune" : values,
                    x: element?.x ?? 0,
                    y: element?.y ?? 0,
        ...
        await this.pdfService.editAndCopy(this.auditTemplatePath, `output/${leaseID}.pdf`, auditEntriesPDF);
    }
    ...
    public async start() {
        for await (const lease of this.leasesPath) {
            const file = await this.prisma.lease.create({
                data: {
                    leaseName: this.leaseFileName[this.leasesPath.indexOf(lease)].split('.').slice(0, -1).join('.') as string,
                    processId: this.ID,
        ...
        await this.excelsService.writeExcel(this.processExcels, this.excelsTemplatePath, `output/${this.ID}.xlsx`);        
     
        return {
            processId: this.ID,
            excel: `http://localhost:3000/output/${this.ID}.xlsx`,
            leases: this.leaseOutputPath.map((path) => `http://localhost:3000/output/${path.split('/').pop()}`)
        }
    }
}
