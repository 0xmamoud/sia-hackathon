    ...
    private async leaseGetAudit(leaseID: number, index: number, lease: string, auditScan: AuditCase[]) {
        ...
        for (let use_case of auditScan) {
            ...
            const response = {'element_1': 'test_1',
                ...
                'element_162': 'test_162',
                'element_200': 'test_200',
                'element_201': 'test_201',
                'element_202': 'test_202',
                'element_203': 'test_203',
                'element_204': 'test_204',
                'element_205': 'test_205',
                'element_206': 'test_206',
            } as any;
    ...
    public async start() {
        ...
        for await (const lease of this.leasesPath) {
            const file = await this.prisma.lease.create({
                data: {
                    leaseName: this.leaseFileName[this.leasesPath.indexOf(lease)].split('.').slice(0, -1).join('.'),
                    processId: this.ID,
                    templateAuditPath: this.auditTemplatePath.split('/').pop(),
                    templateExcelPath: this.excelsTemplatePath.split('/').pop(),
                },
            });
        ...
        this.excelsTemplatePath = `output/lease_excel/${this.ID}.xlsx`;
        
     
        return {
            excel: this.excelsTemplatePath,
            leases: this.leaseOutputPath
        }
    }
}
