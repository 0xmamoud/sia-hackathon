...
export class AIProcessService {
    ...
    private async auditScan() {
        ...
        // console.log(await this.pdfService.extractTextWithCoordinates(this.auditTemplatePath));
    ...
    private async leaseGetExcel(index: number, lease: string, excelsScan: any) {
        const message = [
            {
                role: 'system',
        ...
        this.processExcels.push(...sheetValues);
    }

    private async leaseGetAudit(index: number, lease: string, auditScan: IPDFContent[]) {
        const message = [
            {
                role: 'system',
                content: "Vous êtes un expert en analyse de bail commercial spécialisé dans l'analyse de documents. Extrayez les informations demandées et répondez UNIQUEMENT avec un JSON valide dans le format spécifié Analysez le document fourni et extrayez les informations clés du bail commercial.\n" +
                    "\n" +
                    "FORMAT DE RÉPONSE REQUIS :\n" +
                    this.pdfService.createPromptElement(auditScan) +
                    "\n" +
                    "RÈGLES D'EXTRACTION :\n" +
                    "- Si l'information est absente : utiliser null\n" +
                    "- Pour les dates : format JJ/MM/AAAA\n" +
        ...
        const response = {
            "0": {
                "0": "172 re Raimon de Trencavel Le Clos des Muses CS 40066 34075 MONTPELLIER CEDEX 3",
                "1": "Laélé",
                "2": "EL",
                "3": "01/10/2016",
                "4": "01/10/2025",
        ...
        const entries = Object.keys(response).map((key) => {
            ...
            const etr = Object.keys(values).map((k) => {
                return {
                    page: parseInt(key),
                    text: values[k] ?? "Aucune information",
                    x: auditScan[parseInt(key)].labels[parseInt(k)].x,
                    y: auditScan[parseInt(key)].labels[parseInt(k)].y
                }
            })
        ...
        await this.pdfService.editAndCopy(this.auditTemplatePath, `output/lease_pdf/${this.ID}_${index}.pdf`, entries);
    }

    private async scanLease(leasePath: string, auditScan: IPDFContent[], excelsScan: any){
        const leasePDFtoMD = await this.pdfService.toMd(leasePath);

        await this.leaseGetAudit((this.leasesPath.indexOf(leasePath) + 1), leasePDFtoMD, auditScan);
        await this.leaseGetExcel((this.leasesPath.indexOf(leasePath) + 1), leasePDFtoMD, excelsScan);
    }
